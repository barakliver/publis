    ...
    <ul>
      ${isStatic
        ? `<li>עברו על הפריטים, סמנו, וכתבו הערות. הכול נשמר במכשיר שלכם.</li>
           ...
           <li><b>שמירה</b> שומרת את תמונות הקרוסלה כ-JPG לגלריה, לפי הסדר, ומעתיקה את הטקסט.
               משם פותחים אינסטגרם ומעלים ישירות.</li>
...
<dialog id="zoom">
  <div class="lb">
    ...
    <button class="lb-dl" id="lbDl">⬇︎ שמירת התמונות</button>
  </div>
</dialog>
...
function itemHTML(it) {
  ...
  return '<article class="item' + (isCar ? ' carousel' : '') + '" id="it-' + it.id +
    ...
    '<div>' +
      ...
      (ISSTATIC ? (
        '<div class="tools">' +
          '<button class="tool" data-dl="' + it.id + '">⬇︎ שמירת ' +
            (shots.length > 1 ? shots.length + ' התמונות' : 'התמונה') + '</button>' +
...
function paintLightbox() {
  ...
  $('lbDl').textContent = single ? '⬇︎ שמירת התמונה' : '⬇︎ שמירת ' + lb.shots.length + ' התמונות';
}
...
if (ISSTATIC) {
  ...
  $('lbDl').hidden = true;   // saving lives on the hosted build
}
...
/* ---------- saving the images ---------------------------------------------
   A zip is useless on a phone: it cannot be unpacked easily and Instagram
   cannot read one. What is needed is the pictures themselves, in the camera
   roll, in order. So: convert each slide to JPEG and hand them over.

   On a phone that means the share sheet (navigator.share with files) - the
   viewer taps Save Images once and all of them land in Photos, in order.
   Desktop has no share sheet, so it falls back to saving them one by one.
   The caption goes to the clipboard either way, because it is needed at the
   same moment and neither path can carry it into Instagram. */

async function toJpeg(url, quality) {
  const img = new Image();
  img.decoding = 'sync';
  img.src = url;
  await img.decode();
...
function toast(btn, text, ms) {
  const was = btn.dataset.was || btn.textContent;
  btn.dataset.was = was;
  btn.textContent = text;
  clearTimeout(btn._t);
  btn._t = setTimeout(() => { btn.textContent = btn.dataset.was; }, ms || 2600);
}
...
async function downloadItem(id, btn) {
  ...
  try {
    ...
    for (let i = 0; i < imgs.length; i++) {
      const blob = await toJpeg(imgs[i]);
      const name = imgs.length > 1
        ? base + '-' + String(i + 1).padStart(2, '0') + '.jpg'
        : base + '.jpg';
      files.push(new File([blob], name, { type: 'image/jpeg' }));
    }

    // Put the caption where it can be pasted; failures here are not fatal.
    let copied = false;
    try { await navigator.clipboard.writeText(captionText(item)); copied = true; } catch {}

    if (navigator.canShare && navigator.canShare({ files })) {
      await navigator.share({ files, title: base });
      toast(btn, copied ? 'הטקסט הועתק ✓' : 'נשמר ✓');
    } else {
      for (const f of files) {
        saveBlob(f, f.name);
        await new Promise((r) => setTimeout(r, 350));   // browsers throttle bursts
      }
      toast(btn, files.length + (copied ? ' תמונות + טקסט ✓' : ' תמונות ✓'));
    }
  } catch (err) {
    if (err && err.name === 'AbortError') return;        // the viewer cancelled
    toast(btn, 'לא הצליח — נסו שוב', 3200);
  } finally {
